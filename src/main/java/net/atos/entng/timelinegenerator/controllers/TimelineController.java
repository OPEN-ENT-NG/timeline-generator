							...
							create(request, createRes -> {
								if (createRes.succeeded()) {
									...
									explorerPlugin.notifyUpsert(user, newTimeline, folderId)
											.onSuccess(e -> renderJson(request, createdTimeline))
											.onFailure(e -> renderErrorWithMessage(request, Operation.CREATE, createdTimeline.getString("_id"), e.getMessage()));
								} else {
									renderErrorWithMessage(request, Operation.CREATE, data.getString("headline"), createRes.cause().getMessage());
								}
							})
				))
						...
						((DefaultTimelineService) timelineService).update(id, data, updateRes -> {
							if (updateRes.isRight()) {
								...
								explorerPlugin.notifyUpsert(user, data)
										.onSuccess(e -> renderJson(request, updateRes.right().getValue()))
										.onFailure(e -> renderErrorWithMessage(request, Operation.UPDATE, id, e.getMessage()));
							} else {
					...
					((DefaultTimelineService) timelineService).delete(id, deleteRes -> {
						if (deleteRes.isRight()) {
							...
							explorerPlugin.notifyDeleteById(user, new IdAndVersion(id, System.currentTimeMillis()))
									.onSuccess(onSuccess -> renderJson(request, deleteRes.right().getValue()))
									.onFailure(error -> renderErrorWithMessage(request, Operation.DELETE, id, error.getMessage()));
						} else {
